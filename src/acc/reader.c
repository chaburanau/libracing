...
void acc_read_entry_list_car(acc_car_info_t *data, char *buffer, size_t *offset) {
    ...
    data->driver_infos.size = (uint32_t)size;
    data->driver_infos.data = malloc(sizeof(acc_driver_info_t) * (size_t)size);

    for (uint8_t i = 0; i < size; i++) {
        acc_read_string(&data->driver_infos.data[i].first_name, buffer, offset);    // 9. First Name
        acc_read_string(&data->driver_infos.data[i].last_name, buffer, offset);     // 10. Last Name
        acc_read_string(&data->driver_infos.data[i].short_name, buffer, offset);    // 11. Short Name
        acc_read_uint8_t(&data->driver_infos.data[i].category, buffer, offset);     // 12. Category
        acc_read_uint16_t(&data->driver_infos.data[i].nationality, buffer, offset); // 13. Nationality
    }
}
