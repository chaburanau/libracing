...
void acc_write_uint8_t(char *buffer, size_t *offset, uint8_t data) {
    acc_write_type(buffer, offset, &data, sizeof(uint8_t));
}
...
int32_t acc_write_register_application(char **buffer, size_t *total, acc_reg_app_t *reg_app) {
    ...
    *buffer = malloc(*total);
    ...
    size_t offset = 0;
    acc_write_uint8_t(*buffer, &offset, ACC_OUTBOUND_MESSAGE_REGISTER_COMMAND_APPLICATION);
    acc_write_uint8_t(*buffer, &offset, ACC_BROADCASTING_PROTOCOL_VERSION);
...
int32_t acc_write_unregister_application(char **buffer, size_t *total, acc_unreg_app_t *unreg_app) {
    ...
    *buffer = malloc(*total);
    ...
    size_t offset = 0;
    acc_write_uint8_t(*buffer, &offset, ACC_OUTBOUND_MESSAGE_UNREGISTER_COMMAND_APPLICATION);
...
int32_t acc_write_request_entry_list(char **buffer, size_t *total, acc_req_entry_list_t *req_entry_list) {
    ...
    *buffer = malloc(*total);
    ...
    size_t offset = 0;
    acc_write_uint8_t(*buffer, &offset, ACC_OUTBOUND_MESSAGE_REQUEST_ENTRY_LIST);
...
int32_t acc_write_request_track_data(char **buffer, size_t *total, acc_req_track_data_t *req_track_data) {
    ...
    *buffer = malloc(*total);
    ...
    size_t offset = 0;
    acc_write_uint8_t(*buffer, &offset, ACC_OUTBOUND_MESSAGE_REQUEST_TRACK_DATA);
...
int32_t acc_write_change_hud_page(char **buffer, size_t *total, acc_change_hud_page_t *change_hud_page) {
    ...
    *buffer = malloc(*total);
    ...
    size_t offset = 0;
    acc_write_uint8_t(*buffer, &offset, ACC_OUTBOUND_MESSAGE_CHANGE_HUD_PAGE);
...
int32_t acc_write_change_focus(char **buffer, size_t *total, acc_change_focus_t *change_focus) {
    ...
    (*total) += sizeof(uint8_t); // 1. Request Type
    (*total) += sizeof(int32_t); // 2. Connection ID
    (*total) += sizeof(uint8_t); // 3.1. Car Index Indicator
    if (has_car_index) {
    ...
    *buffer = malloc(*total);
    ...
    size_t offset = 0;
    acc_write_uint8_t(*buffer, &offset, ACC_OUTBOUND_MESSAGE_CHANGE_FOCUS);
...
int32_t acc_write_instant_replay_request(char **buffer, size_t *total, acc_req_instant_replay_t *req_instant_replay) {
    ...
    *buffer = malloc(*total);
    ...
    size_t offset = 0;
    acc_write_uint8_t(*buffer, &offset, ACC_OUTBOUND_MESSAGE_INSTANT_REPLAY_REQUEST);
