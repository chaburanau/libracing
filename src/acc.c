...
int acc_write_string(char *buffer, int offset, const string_t *string) {
    if (string == NULL) {
		return acc_write_uint16_t(buffer, offset, 0);
	}

    offset = acc_write_uint16_t(buffer, offset, string->size);
...
int32_t acc_write_change_focus(char **buffer, const acc_change_focus_t *change_focus) {
    ...
    if (has_car_index) total += sizeof(uint16_t);					// 3.2. Car Index
    total += sizeof(int8_t);										// 4.1. Camera Indicator
    if (has_cameras) total += change_focus->camera_set->size * 2;	// 4.2. Camera Set
    if (has_cameras) total += change_focus->camera->size * 2;		// 4.2. Camera
    ...
    *buffer = malloc(total);
    ...
    offset = acc_write_int8_t(*buffer, offset, has_car_index ? 1 : 0);
    if (has_car_index) {
        offset = acc_write_uint16_t(*buffer, offset, *change_focus->car_index);
    }

	offset = acc_write_int8_t(*buffer, offset, has_cameras ? 1 : 0);
    if (has_cameras) {
...
int32_t acc_read_track_data(acc_track_data_t *data, const char *buffer) {
    int32_t offset = 1;
    ...
    data->camera_sets->size = camera_set_size;
    data->camera_sets->data = malloc(sizeof(string_t) * camera_set_size);


    for (int camera_set_index = 0; camera_set_index < camera_set_size; camera_set_index++) {
        int8_t camera_size = 0;
        int32_t previous_camera_size = data->cameras->size;
        ...
        data->cameras->size += camera_size;
        data->cameras->data = realloc(data->cameras->data, sizeof(string_t) * data->cameras->size);

        for (int camera_index = 0; camera_index < camera_size; camera_index++) {
    ...
    data->hud_pages->size = hud_page_size;
    data->hud_pages->data = malloc(sizeof(string_t) * hud_page_size);
...
int32_t acc_read_broadcasting_event(acc_broadcasting_event_t *data, const char *buffer) {
    int32_t offset = 1;
