...
typedef struct {
    ...
    ac_operation_t operation; // Type of the operation. See `ac_operation_t`
} ac_request_t;

typedef struct {
    char car_name[100];     // Name of the car that player is driving
    char driver_name[100];  // Name of the driver playing
    int32_t identifier;     // For now, it is just 4242; different status
    int32_t version;        // For now, it is just 1; version the server is running
    char track_name[100];   // Name of the track that is running
    char track_config[100]; // Name of the track configuration that is running
} ac_response_t;
...
typedef struct {
    ...
    char driver_name[100]; // Driver name of the car
    char car_name[100];    // Car name
...
int32_t ac_get_last_error(void);
